      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          ...
          function_b.vector_value(fe_values.quadrature_point(q),
                                    b_fun_loc);
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i,j) -= scalar_product( b_tensor , fe_values.shape_grad(i,q) ) *fe_values.shape_value(j,q) * fe_values.JxW(q);
                  ...
                  for (unsigned int q = 0; q < n_q_face; ++q)
                    {
                      for (unsigned int i = 0; i < dofs_per_cell; ++i)
                        {
                          cell_rhs(i) +=
                            function_phi.value(fe_values.quadrature_point(q))
